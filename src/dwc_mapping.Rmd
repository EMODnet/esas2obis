...
# Import data
DBI::dbWriteTable(db, "campaigns", campaigns)
DBI::dbWriteTable(db, "samples", samples)
DBI::dbWriteTable(db, "positions", positions)
DBI::dbWriteTable(db, "observations", observations)

# Import code lists
DBI::dbWriteTable(db, "species", species)
DBI::dbWriteTable(db, "shipc", shipc)
DBI::dbWriteTable(db, "platformclass", platformclass)
DBI::dbWriteTable(db, "platformside", platformside)
DBI::dbWriteTable(db, "bdcountmethod", bdcountmethod)
DBI::dbWriteTable(db, "targettaxa", targettaxa)
DBI::dbWriteTable(db, "useofbinoculars", useofbinoculars)
DBI::dbWriteTable(db, "beaufort", beaufort)
DBI::dbWriteTable(db, "visibility", visibility)
DBI::dbWriteTable(db, "glare", glare)
DBI::dbWriteTable(db, "cloudcover", cloudcover)
DBI::dbWriteTable(db, "precipitation", precipitation)
DBI::dbWriteTable(db, "sightability", sightability)
DBI::dbWriteTable(db, "observationdistance", observationdistance)
DBI::dbWriteTable(db, "lifestage", lifestage)
DBI::dbWriteTable(db, "moult", moult)
DBI::dbWriteTable(db, "plumage", plumage)
DBI::dbWriteTable(db, "sex", sex)
DBI::dbWriteTable(db, "traveldirection", traveldirection)
DBI::dbWriteTable(db, "preytype", preytype)
DBI::dbWriteTable(db, "association", association)
DBI::dbWriteTable(db, "behaviour", behaviour)
```

Set filter: **TODO**: remove for production use.

```{r}
campaign_id <- c(110000153)
```
...
```{r}
dwc_emof_sql <- glue::glue_sql(readr::read_file(here::here("sql", "dwc_emof.sql")), .con = db)
dwc_emof <- DBI::dbGetQuery(db, dwc_emof_sql)
```
