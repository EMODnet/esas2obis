...
For now, the source data were provided manually and stored in `data/raw`.

Read source files:

```{r}
# All columns are read as characters to easy import into SQLite: https://stackoverflow.com/a/13462536/2463806
...
```{r eval=FALSE}
readr::write_csv(species, here::here("reference", "species.csv"), na = "")
readr::write_csv(behaviour, here::here("reference", "behaviour.csv"), na = "")
readr::write_csv(shipc, here::here("reference", "shipc.csv"), na = "")
```

Read code lists:

```{r}
species <- readr::read_csv(here::here("reference", "species.csv"), col_types = cols(.default = "c"))
behaviour <- readr::read_csv(here::here("reference", "behaviour.csv"), col_types = cols(.default = "c"))
shipc <- readr::read_csv(here::here("reference", "shipc.csv"), col_types = cols(.default = "c"))
```
...
Convert dates to characters, to avoid issues [when reading from SQLite](https://stackoverflow.com/a/13462536/2463806):

```{r}
campaigns$StartDate <- as.character(campaigns$StartDate)
campaigns$EndDate <- as.character(campaigns$EndDate)
...
Set filter **TO BE REMOVED HERE AND IN SQL**:
